...
bool Scheduler::DistanceLimitScheduleValidationAlgorithm::isValid(const Schedule& schedule) const
{
	...
	return DistanceAccumulator::accumulateDistance(schedule.getRuns().front().get().getStops().begin(), schedule.getRuns().back().get().getStops().end()) > schedule.constraints().scheduleDistanceLimit();
}
