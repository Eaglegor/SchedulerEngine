...
namespace Scheduler
{
	...
	void Run::setVehicle(Optional<const Vehicle&> vehicle)
	{
		...
		if(!vehicle && this->vehicle)
			context.arrival_time_actualizer.setDirty(true);
		else if(vehicle && !this->vehicle)
			context.arrival_time_actualizer.setDirty(true);
		else if(vehicle && this->vehicle && vehicle->getRoutingProfile( ) != this->vehicle->getRoutingProfile( ))
			context.arrival_time_actualizer.setDirty(true);
	...
	void Run::spliceWorkStops(ConstWorkStopIterator pos, Run& from, ConstWorkStopIterator first, ConstWorkStopIterator last, Optional<std::size_t> n)
	{
		...
		if(n)
			casted_work_stops.splice(pos, from.casted_work_stops, first, last, n.get( ));
		else
			casted_work_stops.splice(pos, from.casted_work_stops, first, last);
